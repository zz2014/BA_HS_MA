...
Java and Node.js are two of the most popular languages for web oriented applications. While Java is capable of multi-threading and CPU efficient, Node.js is single-threaded and uses an event-driven, non-blocking I/O model. The thesis conducted a series of load tests on the two technologies to compare how well they handle high loads and find the best strategy to scale them in Cloud Foundry. Both technologies show a quite high performance in the context of single transaction operations. It is safe to say they are both good candidates to implement applications of SAP business use case. Node.js is found excellent in CPU and memory utilization. With its single thread architecture, horizontal scaling is the best choice. Although Java is more memory-consuming, it is capable of utilizing multiple CPUs in one single application. It is best suitable for vertical scaling if it can make good use of all available CPU resource through proper optimization. 